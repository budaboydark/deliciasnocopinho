...
$IDs = $_POST['print_id'];
foreach ($IDs as $id) {
	$tcf = $this->db->select('*')->from('pref_processofiscal_tcf')->where(array('id' => $id))->get()->first_row('array');
	
	$ordemfiscal = $this->db->select('*')->from('pref_processofiscal_ordemfiscal')->where(array('id' => $tcf['idordemfiscal']))->get()->first_row('array');
    ...
    <table width="100%" cellspacing="0" >
        <tr>
          <td style="background-color:#CCCCCC;border:1px solid black;font-size:18px;padding:5px;text-align: center;" colspan="2">TERMO DE CONCLUS&Atilde;O DE FISCALIZA&Ccedil;&Atilde;O N&deg; <?php echo $id."/".date("Y");?> - TRIBUTO: ISSQN</td></tr>
    </table>
