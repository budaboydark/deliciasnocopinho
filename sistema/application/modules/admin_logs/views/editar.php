<div id="add" class="subcontent">
   	<form id="form1" class="stdform" method="post" action="<?php echo site_url('admin/'.$this->uri->segment(2).'/save'); ?>" enctype="multipart/form-data" >
        ...
        <p>
            ...
            <span class="field">
                <input type="text" name="matricula" id="matricula" class="longinput <?php if($this->error->form_error('matricula') != '') echo 'error'; ?>" value="<?php if(htmlspecialchars($data['matricula']) != '') echo htmlspecialchars($data['matricula']); else echo $this->error->set_value('matricula');  ?>" data-validate="{validate:{required:true, messages:{required:'O campo Nome é obrigat&oacute;rio'}}}" />
        ...
        <p>
        	<label>Nome</label>
            <span class="field">
            	<input type="text" name="name" id="name" class="longinput <?php if($this->error->form_error('name') != '') echo 'error'; ?>" value="<?php if(htmlspecialchars($data['name']) != '') echo htmlspecialchars($data['name']); else echo $this->error->set_value('name');  ?>" data-validate="{validate:{required:true, messages:{required:'O campo Nome é obrigat&oacute;rio'}}}" />
