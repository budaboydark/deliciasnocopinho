...
class authentication_model_contribuinte extends CI_Model {
    ...
    function login($email, $pass, $expire_close = TRUE) {
        ...
        $this->db->where("pref_contribuintes.cnpj = '" . $email4 . "' AND pref_contribuintes.user_group_id IN ({$this->group_id}) AND pref_contribuintes.status = '1' AND pref_contribuintes.type_id = '2' ");
